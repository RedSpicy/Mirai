    ...
    void FixedUpdate()
    {
        if (PlayerData.Instance.bGameStart == true)
        {
            if (PlayerData.Instance.bShowResult == false)
            {
                ...
                if ((PlayerData.Instance.nTime >= 120.0f && PlayerData.Instance.nTime <= 240.0f) ||        //モーニング
                     (PlayerData.Instance.nTime >= 360.0f && PlayerData.Instance.nTime <= 480.0f) ||        //昼
                     (PlayerData.Instance.nTime >= 720.0f && PlayerData.Instance.nTime <= 900.0f))          //夜
                {
                    // レベル１
